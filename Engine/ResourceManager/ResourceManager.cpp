...
const sf::Texture &ResourceManager::getTexture(const std::string &path) { return getMutTexture(path); }

void ResourceManager::setTextureSmooth(const std::string &path, bool smooth) { getMutTexture(path).setSmooth(smooth); }
...
const sf::SoundBuffer &ResourceManager::getSoundBuffer(const std::string &path) {
	if (!m_sound_buffers.contains(path)) m_sound_buffers[path].loadFromFile(path);
