...
namespace Math {
	...
	float _determinant(const Vector2f &tail, const Vector2f &head1,
	                   const Vector2f &head2) {
		return (head1.x - tail.x) * (head2.y - tail.y)
		     - (head2.x - tail.x) * (head1.y - tail.y);
	}

	bool isPointInsideConvex(const std::vector<Vector2f> &convex,
		...
		for (std::size_t i = 0; i < convex.size(); i++) {
			int now_sign = sign(_determinant(point, convex[i],
			                                 convex[(i + 1) % convex.size()]));
	...
	bool doShapesIntersect(const std::vector<Vector2f> &shape1,
	                       const std::vector<Vector2f> &shape2) {
		...
		for (size_t t = 0; t < 2; ++t) {
			...
			for (size_t i = 0; i < s1->size(); i++) {
				Vector2f perpendicular = _getPerpendicular(
					Vector2f((*s1)[(i + 1) % s1->size()].x - (*s1)[i].x,
	...
	Vector2f findLineIntersection(Vector2f p1, Vector2f p2, Vector2f p3,
	                              Vector2f p4) {
		float px
