...
namespace mk {
	...
	public:
		...
		std::map<unsigned int, std::list<std::unique_ptr<WorldEntity>>>
			m_entity_pool;
		...
		void addChild(
			std::unique_ptr<WorldEntity> entity, unsigned int drawOrder = 0
		);

		// ugly, but this function has to be in .hpp file, otherwise it's
		// unusable
		...
		void
			renderOnto(sf::RenderTarget &target, sf::RenderStates states) const;
