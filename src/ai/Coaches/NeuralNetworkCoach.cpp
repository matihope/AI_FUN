...
namespace ai {
	...
	ai::NeuralNetworkCoach::NeuralNetworkCoach(
		...
		mut.push_back({});
		for (uint layerId = 1; layerId < neuralNetwork.getLayerSizes().size();
		     layerId++)
			mut.push_back(std::vector<std::mutex>(neuralNetwork.getLayerSizes(
			)[layerId]));
	}

	void ai::NeuralNetworkCoach::trainSingle(
		const TrainingSet &trainingSet, double learnRate
	) {
		// Create gradients
		gradients.clear();
		...
		for (const auto &item : trainingSet)
			threads.emplace_back(
				&ai::NeuralNetworkCoach::updateGradients, this, item
			);
		...
		//			for (int nodeId = 0; nodeId < layer.getNodeCount();
		//++nodeId) 				std::cout << layerId << "," << nodeId
		//						  << ", diff in bias: " <<
		// layers[layerId].getBias(nodeId)
		//- layer.getBias(nodeId) <<
		//'\n';
		//		}
	}
	...
	void ai::NeuralNetworkCoach::updateGradients(const TrainingItem &item) {
		...
		updateGradient(
			nodeValues, gradients.back(), outputs.activations[layerCount - 2]
		);

		// Hidden layers node values
		for (int layerIndex = layerCount - 2; layerIndex > 0; layerIndex--) {
			nodeValues = std::move(
				calculateHiddenLayerNodeValues(layerIndex, outputs, nodeValues)
			);
			updateGradient(
				nodeValues,
	...
	void ai::NeuralNetworkCoach::updateGradient(
		...
		for (uint node = 0; node < gradient.getLayer().getNodeCount(); node++) {
			...
			for (uint inputNode = 0;
			     inputNode < gradient.getLayer().getInputNodeCount();
			     inputNode++)
	...
	void ai::NeuralNetworkCoach::applyGradients(
		std::vector<LayerGradient> &gradients, double learnRate
	) {
	...
	std::vector<double> ai::NeuralNetworkCoach::calculateOutputLayerNodeValues(
		ai::NeuralNetworkCalculationState &outputs, const ai::TrainingItem &item
	) {
		auto activatingFunction
			= neuralNetwork.getLastLayerActivatingFunction();

		std::vector<double> nodeValues;


		if (activatingFunction->getName() == "SoftMax"
		    && costFunction->getName() == "CrossEntropy") {
			for (int i = 0; i < neuralNetwork.getLayerSizes().back(); i++)
				nodeValues.push_back(
					outputs.activations.back()[i] - item.correctOutput[i]
				);
			return nodeValues;
		}

		auto activatingFunctionDerivatives
			= activatingFunction->derivative(outputs.weightedInputs.back());
		auto costFunctionDerivatives = costFunction->derivative(
			outputs.activations.back(), item.correctOutput
		);

		for (int i = 0; i < neuralNetwork.getLayerSizes().back(); i++)
			nodeValues.push_back(
				activatingFunctionDerivatives[i] * costFunctionDerivatives[i]
			);
		return nodeValues;
	}
	...
	std::vector<double> ai::NeuralNetworkCoach::calculateHiddenLayerNodeValues(
		...
		auto derivatives = neuralNetwork.getActivatingFunction()->derivative(
			outputs.weightedInputs[layerIndex]
		);
	...
	double NeuralNetworkCoach::train(
		...
		for (uint batchId = 0; batchId < trainingSet.size() / batchSize;
		     ++batchId) {
			TrainingSet batch;
		...
		for (uint epoch = 0; epoch < epochs; epoch++) {
			for (const auto &batch : batches)
				trainSingle(batch, trainingFactor);
