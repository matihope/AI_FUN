...
namespace ai {
	LayerGradient::LayerGradient(Layer &layer): layer(layer) {
		weightGradient.resize(
			layer.getNodeCount(),
			std::vector<double>(layer.getInputNodeCount(), 0)
		);
	...
	void LayerGradient::apply(double learnRate, double lambda) {
		for (uint node = 0; node < layer.getNodeCount(); node++) {
			for (uint inputNode = 0; inputNode < layer.getInputNodeCount();
			     inputNode++) {
				...
				layer.setWeight(
					...
					inputNode,
					weight
						- (weightGradient[node][inputNode] - weight * lambda)
							  * learnRate
				);
			}
			auto bias = layer.getBias(node);
			layer.setBias(
				node, bias - (biasGradient[node] - bias * lambda) * learnRate
			);
		}
	}
