...
namespace ai {
	std::vector<double> Layer::calculate(const std::vector<double> &activations
	) const {
		...
		for (int node = 0; node < nodes; node++) {
			...
			for (int inputNode = 0; inputNode < inputs; inputNode++)
				output[node]
					+= weights[node][inputNode] * activations[inputNode];
		}
		return output;
	}

	Layer::Layer(uint nodes, uint inputs, uint id):
		  nodes(nodes),
		  inputs(inputs),
		  id(id) {
	...
	void Layer::randomizeWeightsAndBiases() {
		...
		for (int node = 0; node < nodes; node++)
			for (int input = 0; input < inputs; input++)
				weights[node][input]
					= mk::Random::getNormalDist(0.0, 1.0) / inputs;
	}
