...
void LayerGradient::apply(double trainingFactor) {
  for(uint node = 0; node < layer.getNodesCount(); node++) {
    for(uint inputNode = 0; inputNode < layer.getInputNodesCount(); inputNode++) {
      ...
      layer.setWeight(node, inputNode, weight - weightGradient[node][inputNode] * trainingFactor);
      layer.setBias(node, inputNode, bias - biasGradient[node][inputNode] * trainingFactor);
    }
  }
}
