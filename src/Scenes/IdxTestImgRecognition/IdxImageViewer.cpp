...
IdxImageViewer::IdxImageViewer(idx::Reader &data): data(data) {
	setImageIndex(0);
}

void IdxImageViewer::onDraw(sf::RenderTarget &target, sf::RenderStates states)
	const {
...
void IdxImageViewer::setImageIndex(uint newImageId) {
	...
	for (uint y = 0; y < pixels.size(); y++) {
		for (uint x = 0; x < pixels[y].size(); x++) {
			sf::Color color(
				255 - pixels[y][x], 255 - pixels[y][x], 255 - pixels[y][x]
			);
